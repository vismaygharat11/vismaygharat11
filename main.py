...
# Initialize hazard rate lattice
h_lattice = np.zeros((n + 1, n + 1))
for i in range(n + 1):
    for j in range(i + 1):
        h_lattice[i, j] = a * (b ** (j - i/2))

# Initialize bond price lattice
Z_lattice = np.zeros((n + 1, n + 1))

# Terminal condition: Z_ij at maturity is the face value
Z_lattice[n, :] = F

# Backward induction for bond pricing
for i in range(n - 1, -1, -1):
    for j in range(i + 1):
        r_ij = r_lattice[i, j]
        h_ij = h_lattice[i, j]
        # Risk-neutral pricing formula with recovery
        Z_lattice[i, j] = (
            1 / (1 + r_ij) * (
...
# Bond price at the initial node
Z_00 = Z_lattice[0, 0]
print(Z_00)
